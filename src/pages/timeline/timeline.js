import { getUserId, logOut } from '../../firebase/auth.js';
import { accessPost, deletePost } from '../../firebase/firestore.js';
    ...
    const postsTemplate = groupArr.map((post) => {
      ...
      if (userId === post.author) {
        editBtnTemplate = `
          ...
          <button class='btn-edit' id='btn-del' data-author-id=${post.author} data-delete=${post.id}>Excluir</button>
...
/*
  ...
  */
    ...
    btnEdit.addEventListener('click', () => {
      const divModal = container.querySelector('.div-modal');
      console.log('clicou');
      divModal.appendChild(editPosts);
    });
