...
const createNewPost = async (textPost) => {
  const post = {
  ...
  const docReference = await nf(Xa(db, "users", auth.currentUser.uid, "posts"), post);
  post.id = docReference.id;
  return post;
};
const getLoggedUserAllPosts = async () => {
  const postsCollection = await Jl(Xa(db, "users", auth.currentUser.uid, "posts"));
  const posts = [];
