...
const login = () => {
  ...
  botaoEntrar.addEventListener("click", (e) => {
    ...
    fazerLogin(loginEmail.value, loginSenha.value).then(() => {
      ...
      mensagemErro.innerHTML = "Usu\xE1rio ou senha incorretos";
    });
  });
...
const database = async (name2, email) => {
  try {
    const docRef = await Xl(Ja(db, "users"), {
      name: name2,
      email
    });
...
const pegarPost = async (mostrarPost) => {
  await tf(Ja(db, "posts"), (querySnapshot) => {
...
const feed = () => {
  ...
  <main class="feed-post">
    ...
    <div class="texto-ultimos-post">
      <h2 class="ultimos-posts">\xDAltimas publica\xE7\xF5es</h2>
    </div>
  </main>
  ...
  const exibirPostagem = () => {
    ...
    <div class="nome-usuaria-post">
      ...
      <h3>${post2.nome}</h3>
      <p class="dia-post"> ${post2.date}</p>
    </div>  
      <section class="publicacao">
        <p class="titulo-post">T\xEDtulo do Livro: ${post2.titulo}</p>
        <p class="autora-post">Nome da Autora: <strong>${post2.autora}</strong></p>
        <p class="texto-postagem">${post2.post}</p>
      </section> 
    </div>  
