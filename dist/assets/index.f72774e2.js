...
async function createUserWithEmail(email, password, name2) {
  return new Promise((resolve, reject) => {
    createUserWithEmailAndPassword(auth, email, password).then((userCredential) => {
      ...
      Xl(eh(db, "users", auth.currentUser.uid), user);
      updateProfile(auth.currentUser, { displayName: name2 });
    }).catch((error) => {
      const errorMessage = error.message;
      reject(errorMessage);
    }).finally(() => {
      console.log("Processo de cria\xE7\xE3o de conta finalizado em auth.");
    });
  });
}
...
const login = () => {
  ...
  loginWithGoogle.addEventListener("click", () => {
    loginGoogle().then((isAuthenticated) => {
      console.log("google: Usu\xE1rio autenticado em login!");
      window.location.replace("#timeline");
    }).catch((error) => {
      console.log("google: Usu\xE1rio n\xE3o autenticado.");
      window.location.replace("#login");
    }).finally(() => {
      console.log("Processo de autentica\xE7\xE3o do google  finalizado em login.");
    });
  });
  return container;
};
...
const getLoggedUserLikes = async () => {
  const userDocReference = eh(db, "users", auth.currentUser.uid);
  const userDocSnapshot = await jl(userDocReference);
...
const likePosts = async (post, userId) => {
  const postDocReference = eh(db, "posts", post.id);
  const getPostDoc = await jl(postDocReference);
  const likes = getPostDoc.data().likes || [];
  let newLikes;
  if (likes.includes(userId)) {
    newLikes = likes.filter((id2) => id2 !== userId);
  } else {
    newLikes = [...likes, userId];
  }
  await tf(postDocReference, { likes: newLikes });
  const userDocReference = eh(db, "users", userId);
  const getUserDoc = await jl(userDocReference);
  const userLikes = getUserDoc.data().likes || [];
  let userNewLikes;
  if (userLikes.includes(post.id)) {
    userNewLikes = userLikes.filter((id2) => id2 !== post.id);
  } else {
    userNewLikes = [...userLikes, post.id];
  }
  await tf(userDocReference, { likes: userNewLikes });
...
async function editPost(post) {
  ...
  updateButton.addEventListener("click", async () => {
    ...
    if (inputTitle !== "" && inputTextPost !== "") {
      ...
      modalWrapper.classList.remove("show");
      console.log("Fun\xE7\xE3o executada ap\xF3s a edi\xE7\xE3o do post na pagina post.");
      timeline();
    } else {
      alert("Preencha todos os campos");
    }
...
function ListPost() {
  return "<section id='post-list' class='post-list'></section>";
}
const timeline = () => {
  ...
  const template = ` 
    <div class="form-wrapper-timeline">
       <div>        
          ...
          <div class="div-post-type"><button type="button" id="last-posts" class="post-type">\xDAltimos posts</button>
  ...
  function showAllPosts(posts) {
    if (posts) {
      ...
      postsList.innerHTML = postsByDateOrderAsc.map(
        (post) => `
        <article class="post-article">
          <div class="post-header">
            ...
            <div class="div-action-buttons">
              ...
              ${post.like === [] ? `<span class="material-icons like">
              star_border
  ...
  getAllUsersPosts().then((allPosts) => {
    ...
    console.log("Fim da solicita\xE7\xE3o inicial de posts de todos os users.");
  });
  ...
  userPosts.addEventListener("click", () => {
    ...
    getLoggedUserAllPosts().then((allPosts) => {
  ...
  userFavorites.addEventListener("click", () => {
    let userLikedPosts = [];
    getLoggedUserLikes().then((allPosts) => {
      ...
      console.log("Fim da solicita\xE7\xE3o inicial de posts FAVORITOS do user LOGADO.");
    });
  });
      ...
      modalContainer.innerHTML = `    
        ...
        close
          ...
          <div class="delete-top-content">
            <p class="delete-modal-text">Voc\xEA tem certeza de que deseja apagar esse post?</p>   
            
          </div>
          <div class="form-delete">
            <form>
            
            <p class="alert-delete-text">Essa a\xE7\xE3o n\xE3o poder\xE1 ser desfeita e o post ser\xE1 removido.</p>
...
`;
  ...
  registerButton.addEventListener("click", () => {
    ...
    if (email !== "" && password !== "" && name2 !== "") {
      createUserWithEmail(email, password, name2).then((isRegistered) => {
