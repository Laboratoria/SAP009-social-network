...
async function editPost(post) {
  ...
  editButton.addEventListener("click", async () => {
    ...
    if (inputTitle !== "" && inputTextPost !== "") {
      const lulu = await updatePost(inputTitle, inputTextPost, post.id);
      modalWrapper.classList.remove("show");
      console.log("Fun\xE7\xE3o executada ap\xF3s a edi\xE7\xE3o do post.");
      console.log(lulu);
      timeline();
    } else {
      alert("Preencha todos os campos");
...
function ListPost() {
  return "<section id='post-list' class='post-list'><!-- Aqui voc\xEA pode adicionar outros componentes React ou HTML --></section>";
}
const timeline = () => {
  ...
  function showAllPostsAllUsers() {
    if (allUsersPosts) {
      ...
      const postsByDateOrderAsc = mappedPosts.sort((a, b2) => new Date(b2.dateTime.replace(/(\d{2})\/(\d{2})\/(\d{4}), (\d{2}):(\d{2})/, "$2/$1/$3 $4:$5")) - new Date(a.dateTime.replace(/(\d{2})\/(\d{2})\/(\d{4}), (\d{2}):(\d{2})/, "$2/$1/$3 $4:$5")));
